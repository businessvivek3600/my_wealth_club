...
class _SubscriptionPurchaseDialogTestState
  ...
  Future<void> _handleAppLifecycleState(AppLifecycleState state) async {
    ...
    //   if (!available) {
    //     logger.i('AppLifecycleState.resumed not available');
    //     return;
    //   }
    //   logger.i('AppLifecycleState.resumed available');
    //   await getPendingPurchases().then((value) => completePurchases(value));
    // }
  }
...
//////////////////////////////  dummyIds  //////////////////////////////
  ...
  void initState() {
    ...
    _subscription = purchaseUpdated.listen(
      ...
      logger.f('_subscription listener purchaseUpdated',
          tag: tag,
          error:
      ...
      logger.f('_subscription listener purchaseUpdated onDone',
      ...
      logger.e('_subscription listener purchaseUpdated onError',
          tag: tag, error: error);
    });
  ...
  Future<void> getStoreProducts() async {
    ...
    logger.i('initStoreInfo isAvailable: $isAvailable', tag: tag);
    if (!isAvailable) {
      setState(() {
    ...
    logger.i('initStoreInfo _ids: $_ids', tag: tag);
    final ProductDetailsResponse productDetailResponse =
        await _inAppPurchase.queryProductDetails(_ids.toSet());
    logger.i(productDetailResponse.notFoundIDs,
        error:
            'initStoreInfo productDetailResponse:\n found: ${productDetailResponse.productDetails} not-found: ${productDetailResponse.notFoundIDs} error:${productDetailResponse.error}',
        tag: tag);
    ...
    try {
      for (var detail in productDetailResponse.productDetails) {
        logger.i('productDetailResponse.productDetails -> init: ${detail.id}',
            tag: tag);
        for (var element in myPacks) {
          for (var e in element.products) {
            if (e.storeId == detail.id) {
              e.productDetails = detail;
              // _inAppPurchase
        ...
        logger.i(
          'myPacks productDetailResponse.productDetails -> end: ${myPacks.map((e) => e.products.map((e) => e.productDetails?.id))}',
        );
      }
    } catch (e) {
      logger.e('productDetailResponse.productDetails: ${e.toString()}',
          tag: tag);
    }
