...
class FancyTreeView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ChangeNotifierProvider<FancyTeamViewTreeUiProvider>(
          create: (BuildContext context) => FancyTeamViewTreeUiProvider(),
        ),
      ],
      builder: (context, child) => FancyTeamViewUI(),
    );
  }
}
...
class _FancyTeamViewUIState extends State<FancyTeamViewUI> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<FancyTeamViewTreeUiProvider>(
...
//get data from api
  Future<List<TeamDownlineUser>> getDownLines(String level, String id) async {
    print('team-view <getDownLines> ' + level.toString());
    List<TeamDownlineUser> levelArray = [];
    if (isOnline) {
      try {
        ApiResponse apiResponse = await sl
            .get<TeamViewRepo>()
            .getDownLines({'level': level, 'sponser_username': id});
          ...
          try {
            ...
            if (status) {
              try {
                ...
                print('team-view could not generate the level array $e');
              }
            }
          } catch (e) {}
        }
...
class _LazyLoadingTreeViewState extends State<LazyLoadingTreeView> {
  ...
  Widget getLeadingFor(TeamDownlineUser data) {
    ...
    return FolderButton(
      ...
      openedIcon: Icon(Icons.keyboard_arrow_up_rounded, color: Colors.white),
      closedIcon:
